...
import { asignarRegaloPorRuleta, obtenerPedidoPorEnlaceToken } from '@/lib/pedidos';
...
export default function PedidoPublicoPage() {
  ...
  useEffect(() => {
    if (confirmado && pedido?.incluye_regalo && !pedido.regalo_id) {
    ...
    <main className="mx-auto flex min-h-screen max-w-xl flex-col gap-6 px-5 py-10">
      ...
      {!confirmado ? (
        <button
          ...
          className="rounded-tag bg-hilo px-6 py-4 text-lg font-semibold text-white hover:bg-hilo-dark"
        >
          ✅ Confirmar que mi pedido es correcto
        </button>
        ...
        <section className="flex flex-col items-center gap-4 rounded-tag bg-white p-6 shadow-sm ring-1 ring-ink/5">
          ...
          {pedido.regalo_id && pedido.regalo ? (
            <p className="text-lg font-semibold text-hilo-dark">Tu regalo: {pedido.regalo.nombre}</p>
        ...
        <p className="text-center text-ink/50">¡Gracias por tu compra! 💚</p>
      )}
    </main>
  );
}
